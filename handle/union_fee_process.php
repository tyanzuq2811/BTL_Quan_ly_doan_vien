...
function handleAddFee() {
    ...
    $ngay_nop = filter_input(INPUT_POST, 'ngay_nop', FILTER_DEFAULT) ?? null;
    $trang_thai = filter_input(INPUT_POST, 'trang_thai', FILTER_DEFAULT) ?? 'Chưa nộp';
    ...
    if (addFee($doan_vien_id, $loai_doan_phi_id, $nam_hoc, $so_tien, $ngay_nop, $trang_thai)) {
        ...
        header("Location: /BTL/views/union_fee/union_fee_create.php");
        exit();
    }
}
...
function handleEditFee() {
    ...
    if ($id <= 0 || $doan_vien_id <= 0 || $loai_doan_phi_id <= 0 || empty($nam_hoc) || $so_tien <= 0) {
        $_SESSION['error'] = 'Dữ liệu không hợp lệ';
        header("Location: /BTL/views/union_fee/union_fee_edit.php?id=$id");
        exit();
    }
