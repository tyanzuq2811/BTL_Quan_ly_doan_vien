...
function handleAddEvent() {
    $ten_su_kien = filter_input(INPUT_POST, 'ten_su_kien', FILTER_DEFAULT) ?? '';
    $mo_ta = filter_input(INPUT_POST, 'mo_ta', FILTER_DEFAULT) ?? null;
    $ngay_to_chuc = filter_input(INPUT_POST, 'ngay_to_chuc', FILTER_DEFAULT) ?? '';
    $cap_to_chuc = filter_input(INPUT_POST, 'cap_to_chuc', FILTER_DEFAULT) ?? '';
    $cap_id = filter_input(INPUT_POST, 'cap_id', FILTER_VALIDATE_INT) ?? 0;
    $trang_thai = filter_input(INPUT_POST, 'trang_thai', FILTER_DEFAULT) ?? 'Sắp diễn ra';

    if (empty($ten_su_kien) || empty($ngay_to_chuc) || empty($cap_to_chuc) || $cap_id <= 0) {
        $_SESSION['error'] = 'Dữ liệu không hợp lệ';
        header("Location: /BTL/views/event/event_create.php");
        exit();
    }
...
function handleEditEvent() {
    ...
    if ($id <= 0 || empty($ten_su_kien) || empty($ngay_to_chuc) || empty($cap_to_chuc) || $cap_id <= 0) {
        $_SESSION['error'] = 'Dữ liệu không hợp lệ';
        header("Location: /BTL/views/event/event_edit.php?id=$id");
        exit();
    }
