...
if (!$member) {
    header("Location: /BTL/views/member.php?error=Đoàn viên không tồn tại");
    exit();
}
?>
            ...
            <div class="container mt-3">
                <div class="row justify-content-center">
                    <div class="col-md-8">
                        ...
                        <form method="post" action="/BTL/handle/member_process.php?action=edit" onsubmit="return validateForm()">
                            <input type="hidden" name="id" value="<?= htmlspecialchars($member['id'], ENT_QUOTES, 'UTF-8') ?>">
                            <div class="mb-3">
                                <label class="form-label">MSSV</label>
                                <input type="text" name="mssv" class="form-control" value="<?= htmlspecialchars($member['mssv'], ENT_QUOTES, 'UTF-8') ?>" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Họ tên</label>
                                <input type="text" name="ho_ten" class="form-control" value="<?= htmlspecialchars($member['ho_ten'], ENT_QUOTES, 'UTF-8') ?>" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Ngày sinh</label>
                                <input type="date" name="ngay_sinh" class="form-control" value="<?= htmlspecialchars($member['ngay_sinh'], ENT_QUOTES, 'UTF-8') ?>" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Giới tính</label>
                                <select name="gioi_tinh" class="form-select" required>
                                    <option value="Nam" <?= $member['gioi_tinh'] == 'Nam' ? 'selected' : '' ?>>Nam</option>
                                    <option value="Nữ" <?= $member['gioi_tinh'] == 'Nữ' ? 'selected' : '' ?>>Nữ</option>
                                    <option value="Khác" <?= $member['gioi_tinh'] == 'Khác' ? 'selected' : '' ?>>Khác</option>
                                </select>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Email</label>
                                <input type="email" name="email" class="form-control" value="<?= htmlspecialchars($member['email'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Số điện thoại</label>
                                <input type="text" name="so_dien_thoai" class="form-control" value="<?= htmlspecialchars($member['so_dien_thoai'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Địa chỉ</label>
                                <textarea name="dia_chi" class="form-control"><?= htmlspecialchars($member['dia_chi'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Ngày vào đoàn</label>
                                <input type="date" name="ngay_vao_doan" class="form-control" value="<?= htmlspecialchars($member['ngay_vao_doan'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                            </div>
                            <div class="mb-3">
                                ...
                                <select name="chi_doan_id" class="form-select">
                                    ...
                                    <?php foreach ($teams as $team): ?>
                                        <option value="<?= htmlspecialchars($team['id'], ENT_QUOTES, 'UTF-8') ?>" <?= $team['id'] == $member['chi_doan_id'] ? 'selected' : '' ?>>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="trang_thai" class="form-select">
                                    <option value="Hoạt động" <?= $member['trang_thai'] == 'Hoạt động' ? 'selected' : '' ?>>Hoạt động</option>
                                    <option value="Chuyển đi" <?= $member['trang_thai'] == 'Chuyển đi' ? 'selected' : '' ?>>Chuyển đi</option>
                                    <option value="Nghỉ sinh hoạt" <?= $member['trang_thai'] == 'Nghỉ sinh hoạt' ? 'selected' : '' ?>>Nghỉ sinh hoạt</option>
                                </select>
                            </div>
