...
if (!$participation) {
    header("Location: /BTL/views/event_participation.php?error=Tham gia sự kiện không tồn tại");
    exit();
}
?>
...
<head>
    ...
    <title>Edit Event Participation</title>
...
<body>
            ...
            <div class="container mt-3">
                <div class="row justify-content-center">
                    <div class="col-md-6">
                        ...
                        <form method="post" action="/BTL/handle/event_participation_process.php?action=edit" onsubmit="return validateForm()">
                            <input type="hidden" name="id" value="<?= htmlspecialchars($participation['id'], ENT_QUOTES, 'UTF-8') ?>">
                            <div class="mb-3">
                                <label class="form-label">Sự kiện</label>
                                <select name="su_kien_id" class="form-select" required>
                                    <option value="">-- Chọn sự kiện --</option>
                                    <?php foreach ($events as $event): ?>
                                        <option value="<?= htmlspecialchars($event['id'], ENT_QUOTES, 'UTF-8') ?>" <?= $event['id'] == $participation['su_kien_id'] ? 'selected' : '' ?>>
                                            <?= htmlspecialchars($event['ten_su_kien'], ENT_QUOTES, 'UTF-8') ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="mb-3">
                                ...
                                <select name="doan_vien_id" class="form-select" required>
                                    ...
                                    <?php foreach ($members as $member): ?>
                                        <option value="<?= htmlspecialchars($member['id'], ENT_QUOTES, 'UTF-8') ?>" <?= $member['id'] == $participation['doan_vien_id'] ? 'selected' : '' ?>>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="trang_thai" class="form-select">
                                    <option value="Đã đăng ký" <?= $participation['trang_thai'] == 'Đã đăng ký' ? 'selected' : '' ?>>Đã đăng ký</option>
                                    <option value="Đã tham gia" <?= $participation['trang_thai'] == 'Đã tham gia' ? 'selected' : '' ?>>Đã tham gia</option>
                                    <option value="Vắng mặt" <?= $participation['trang_thai'] == 'Vắng mặt' ? 'selected' : '' ?>>Vắng mặt</option>
                                </select>
                            </div>
                            <button type="submit" class="btn btn-success">Cập nhật</button>
                            <a href="/BTL/views/event_participation.php" class="btn btn-secondary">Hủy</a>
                        </form>
                    </div>
                </div>
            </div>
    ...
    <script>
        function validateForm() {
            const suKienId = document.querySelector('select[name="su_kien_id"]').value;
