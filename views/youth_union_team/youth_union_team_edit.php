...
if (!$team) {
    header("Location: /BTL/views/youth_union_team.php?error=Chi đoàn không tồn tại");
    exit();
}
?>
            ...
            <div class="container mt-3">
                <div class="row justify-content-center">
                    <div class="col-md-6">
                        ...
                        <form method="post" action="/BTL/handle/youth_union_team_process.php?action=edit" onsubmit="return validateForm()">
                            <input type="hidden" name="id" value="<?= htmlspecialchars($team['id'], ENT_QUOTES, 'UTF-8') ?>">

                            <div class="mb-3">
                                <label class="form-label">Tên chi đoàn</label>
                                <input type="text" name="ten" class="form-control" value="<?= htmlspecialchars($team['ten'], ENT_QUOTES, 'UTF-8') ?>" required>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="lien_chi_id" class="form-select" required>
                                    ...
                                    <?php foreach ($chapters as $c): ?>
                                        <option value="<?= htmlspecialchars($c['id'], ENT_QUOTES, 'UTF-8') ?>" <?= $c['id'] == $team['lien_chi_id'] ? 'selected' : '' ?>>
                                            <?= htmlspecialchars($c['ten'], ENT_QUOTES, 'UTF-8') ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" name="ngay_thanh_lap" class="form-control" value="<?= htmlspecialchars($team['ngay_thanh_lap'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                            </div>

                            <div class="mb-3">
                                <label class="form-label">Trạng thái</label>
                                <select name="trang_thai" class="form-select">
                                    <option value="Hoạt động" <?= $team['trang_thai'] == 'Hoạt động' ? 'selected' : '' ?>>Hoạt động</option>
                                    <option value="Ngừng hoạt động" <?= $team['trang_thai'] == 'Ngừng hoạt động' ? 'selected' : '' ?>>Ngừng hoạt động</option>
                                </select>
                            </div>
