...
$discipline = getDisciplineById($id);
$members = getAllMembers();
$discipline_types = getAllDisciplineTypes();

if (!$discipline) {
    header("Location: /BTL/views/discipline.php?error=Kỷ luật không tồn tại");
    exit();
}
?>
...
<body>
            ...
            <div class="container mt-3">
                <div class="row justify-content-center">
                    <div class="col-md-6">
                        ...
                        <form method="post" action="/BTL/handle/discipline_process.php?action=edit" onsubmit="return validateForm()">
                            <input type="hidden" name="id" value="<?= htmlspecialchars($discipline['id'], ENT_QUOTES, 'UTF-8') ?>">
                            <div class="mb-3">
                                ...
                                <select name="doan_vien_id" class="form-select" required>
                                    ...
                                    <?php foreach ($members as $member): ?>
                                        <option value="<?= htmlspecialchars($member['id'], ENT_QUOTES, 'UTF-8') ?>" <?= $member['id'] == $discipline['doan_vien_id'] ? 'selected' : '' ?>>
                            ...
                            <div class="mb-3">
                                <label class="form-label">Loại kỷ luật</label>
                                <select name="loai_id" class="form-select" required onchange="updateDescription()">
                                    <option value="">-- Chọn loại kỷ luật --</option>
                                    <?php foreach ($discipline_types as $type): ?>
                                        <option value="<?= htmlspecialchars($type['id'], ENT_QUOTES, 'UTF-8') ?>" data-description="<?= htmlspecialchars($type['mo_ta'] ?? '', ENT_QUOTES, 'UTF-8') ?>" <?= $type['id'] == $discipline['loai_id'] ? 'selected' : '' ?>>
                            ...
                            <div class="mb-3">
                                <label class="form-label">Mô tả loại kỷ luật</label>
                                <textarea id="loai_mo_ta" class="form-control" rows="3" readonly><?= htmlspecialchars($discipline['loai_mo_ta'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Ngày quyết định</label>
                                <input type="date" name="ngay_quyet_dinh" class="form-control" value="<?= htmlspecialchars($discipline['ngay_quyet_dinh'], ENT_QUOTES, 'UTF-8') ?>" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Nội dung</label>
                                <textarea name="noi_dung" class="form-control" rows="4"><?= htmlspecialchars($discipline['noi_dung'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
                            </div>
                            <button type="submit" class="btn btn-success">Cập nhật</button>
                            <a href="/BTL/views/discipline.php" class="btn btn-secondary">Hủy</a>
                        </form>
                    </div>
                </div>
            </div>
    ...
    <script>
        function validateForm() {
            ...
            if (!loaiId) {
                alert('Vui lòng chọn loại kỷ luật.');
                return false;
            }
            if (!ngayQuyetDinh) {
